...
def wait_operation(token: str, operation: dict[str, Any], timeout: int = 60) -> dict[str, Any]:
    ...
    if not op_id:
        raise RuntimeError("requestNew returned an operation without id")
    ...
    while time.time() < deadline:
        ...
        time.sleep(2)
        current = _request_json("GET", f"{OPS_API}/{urllib.parse.quote(op_id)}", token)
    raise RuntimeError(f"Timed out waiting for certificate operation {op_id}")
...
def _load_sa_key(raw: str) -> dict[str, Any]:
    data = json.loads(raw)
...
def _token_from_args(args: argparse.Namespace) -> str:
    if args.iam_token_file:
        token = _read_text(args.iam_token_file).strip()
        if not token:
            raise SystemExit("IAM token file is empty")
        return token
    if args.sa_key_file:
        return iam_token_from_sa_key(_load_sa_key(_read_text(args.sa_key_file)))
    if args.iam_token_stdin:
        token = sys.stdin.read().strip()
        if not token:
            raise SystemExit("IAM token stdin is empty")
        return token
    if args.sa_key_stdin:
        return iam_token_from_sa_key(_load_sa_key(sys.stdin.read()))
    raise SystemExit(
